...
```{r setup_13, cache=TRUE, message=FALSE， eval=False}
...
```{r get_census and extract_geometries,eval=FALSE, message=FALSE, warning=FALSE, cache=TRUE, results = 'hide'}
NJCensus <- 
...
```{r add_census_tracts , message = FALSE, warning = FALSE,eval=FALSE}
...
```{r import_weather, message = FALSE, warning = FALSE ,eval=FALSE}
weather.Data <- 
...
```{r plot_weather, catche = TRUE,eval=FALSE}
grid.arrange( ncol=2,
...
```{r panel_length_check , message = FALSE, warning = FALSE,eval=FALSE}
...
```{r create_panel , message = FALSE,eval=FALSE}
trip.panel <- 
  dat_census %>%
...
time lag variables
```{r time_lags , message = FALSE,eval=FALSE}
trip_panel <- 
  trip_panel %>% 
  arrange( interval60) %>% 
...
```{r evaluate_lags , warning = FALSE, message = FALSE,eval=FALSE}
...
```
```{r evaluate_leads , warning = FALSE, message = FALSE,eval=FALSE}
as.data.frame(trip_panel) %>%
    group_by(interval60) %>% 
...
```{r}
animation<-ggplot()+
  geom_sf(data=NJTracts, color="grey80", size=0.01, fill="transparent")+
  geom_sf(data=trip_panel,aes(color=delay_minutes),size=1.2)+
  geom_sf(data=st_union(NJTracts),color="black",size=1.2,fill="transparent")+
  scale_color_manual(values=palette5)+
  labs(title="delay time for 2019.01, NJ",
       subtitle = "one hour interbals:{current_frame}")+
