...
```{r}
trip_panel$holiday <- ifelse(yday(trip_panel$scheduled_time)%in% c(
  #New Years
  "365", "1", "2", 
  # St. Patrick's Day
  "76",
...
split
```{r}
trip.Train <- filter(trip_panel, week < 5)
trip.Test <- filter(trip_panel, week >= 5)
```
...
plot the bike share number in time series of both prediction and observation
